...
let rec get : 'a quadtree -> coord -> 'a option =
  fun tree (x, y) ->
   ...
   | Noeud (c,t1,t2,t3,t4) -> 
    ...
    if x > x1 && y > y1 then get t2 (x, y) else
    if x > x1 && y < y1 then get t1 (x, y) else
    if x < x1 && y < y1 then get t3 (x, y) 
    else get t4 (x, y) 
...
let rec insert : 'a quadtree -> coord -> 'a -> 'a quadtree =
  fun tree co elem ->
  match tree with
  | Vide case -> Feuille (case, co, elem)
  | Feuille (case, co2, elem2) ->
    if co = co2 then Feuille (case, co, elem)
    else (
      let new_tree = Noeud (case, Vide (nord_ouest case), Vide (nord_est case), 
                     Vide (sud_ouest case), Vide (sud_est case)) in
      let new_tree = insert new_tree co2 elem2 in
      insert new_tree co elem)
  | Noeud (case, t1, t2, t3, t4) ->
    let x, y = co in
    let x1, y1 = centre_case case in
    if x > x1 && y > y1 then Noeud (case, t1,insert t2 co elem, t3, t4) else
    if x > x1 && y < y1 then Noeud (case, insert t1 co elem, t2, t3, t4) else
    if x < x1 && y < y1 then Noeud (case, t1, t2, insert t3 co elem, t4) 
    else Noeud (case, t1, t2, t3, insert t4 co elem)
